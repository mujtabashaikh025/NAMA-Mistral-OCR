...
def extract_text_smart(uploaded_file):
    ...
    # METHOD 2: Mistral Native OCR (Fallback)
    try:
        base64_pdf = encode_pdf(file_bytes)
        
        if 'mistral_client' not in globals() and 'mistral_client' not in locals():
             return f"Mistral Client not initialized. Cannot perform OCR for {uploaded_file.name}."
        ...
        ocr_text = ""
        # Limit to first 3 pages manually to save tokens/cost
